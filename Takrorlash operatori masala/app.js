//================================================================================== 1 - MASALA

// let k = 7;
// let n = 5;

// for (let i = 0; i < n; i++) {
//   console.log(k);
// }

//================================================================================== 1 - MASALA WHILE
// let k = 7;
// let n = 5;
// let i = 0;

// while (i < n) {
//   console.log(k);
//   i++;
// }
//================================================================================== 1 - MASALA DO WHILE

// let k = 7;
// let n = 5;
// let i = 0;

// do {
//   console.log(k);
//   i++;
// } while (i < n);

//2 - MASALA

// let a = 3;
// let b = 8;
// let count = 0;

// for (let i = b - 1; i > a; i--) {
//   console.log(i);
//   count++;
// }

// console.log("Chiqarilgan sonlar soni:", count);

//================================================================================== 2 - MASALA WHILE

// let a = 3;
// let b = 8;
// let count = 0;
// let i = b - 1;

// while (i > a) {
//   console.log(i);
//   count++;
//   i--;
// }

// console.log("Chiqarilgan sonlar soni:", count);

//================================================================================== 2 - MASALA DO WHILE

// let a = 3;
// let b = 8;
// let count = 0;
// let i = b - 1;

// do {
//   if (i > a) {
//     console.log(i);
//     count++;
//   }
//   i--;
// } while (i > a);

// console.log("Chiqarilgan sonlar soni:", count);

//================================================================================== 4 - MASALA

// let narx = 12000;

// for (let i = 1; i <= 10; i++) {
//   console.log(i + " kg narxi: " + (i * narx) + " so'm");
// }
//================================================================================== 4 - MASALA WHILE
// let narx = 12000;
// let i = 1;

// while (i <= 10) {
//   console.log(i + " kg narxi: " + (i * narx) + " so'm");
//   i++;
// }
//================================================================================== 4 - MASALA DO  WHILE
// let narx = 12000;
// let i = 1;

// do {
//   console.log(i + " kg narxi: " + (i * narx) + " so'm");
//   i++;
// } while (i <= 10);

//================================================================================== 5 - MASALA

// let PriceKg = 15000;

// for (let i = 1; i <= 9; i++) {
//   let ogir = i / 10;
//   console.log(ogir + " kg narxi: " + (ogir * PriceKg) + " so'm");
// }
//================================================================================== 5 - MASALA WHILE
// let PriceKg = 15000;
// let i = 1;

// while (i <= 9) {
//   let ogir = i / 10;
//   console.log(ogir + " kg narxi: " + (ogir * PriceKg) + " so'm");
//   i++;
// }
//================================================================================== 5 - MASALA DO WHILE
// let PriceKg = 15000;
// let i = 1;

// do {
//   let ogir = i / 10;
//   console.log(ogir + " kg narxi: " + (ogir * PriceKg) + " so'm");
//   i++;
// } while (i <= 9);

//================================================================================== 6 - MASALA

// let price = 15000;

// for (let ogir = 1.2; ogir <= 2; ogir += 0.2) {
//   console.log(ogir.toFixed(1) + " kg narxi: " + (ogir * price).toFixed(2) + " so'm");
// }

//================================================================================== 6 - MASALA WHILE

// let price = 15000;
// let ogir = 1.2;

// while (ogir <= 2) {
//   console.log(ogir.toFixed(1) + " kg narxi: " + (ogir * price).toFixed(2) + " so'm");
//   ogir += 0.2;
// }

//================================================================================== 6 - MASALA DO WHILE

// let price = 15000;
// let ogir = 1.2;

// do {
//   console.log(ogir.toFixed(1) + " kg narxi: " + (ogir * price).toFixed(2) + " so'm");
//   ogir += 0.2;
// } while (ogir <= 2);

//================================================================================== 7 - MASALA

// let a = 2;
// let b = 6;
// let sum = 0;

// for (let i = a; i <= b; i++) {
//   sum += i;
// }

// console.log("Yig'indisi:", sum);

//================================================================================== 7 - MASALA  WHILE

// let a = 2;
// let b = 6;
// let sum = 0;
// let i = a;

// while (i <= b) {
//   sum += i;
//   i++;
// }

// console.log("Yig'indisi:", sum);

//================================================================================== 7 - MASALA DO WHILE

// let a = 2;
// let b = 6;
// let sum = 0;
// let i = a;

// do {
//   sum += i;
//   i++;
// } while (i <= b);

// console.log("Yig'indisi:", sum);

//================================================================================== 8 -MASALA

// let a = 2;
// let b = 5;
// let product = 1;

// for (let i = a; i <= b; i++) {
//   product *= i;
// }

// console.log("Ko‘paytma:", product);

//================================================================================== 8 - MASALA  WHILE

// let a = 2;
// let b = 5;
// let product = 1;
// let i = a;

// while (i <= b) {
//   product *= i;
//   i++;
// }

// console.log("Ko‘paytma:", product);

//================================================================================== 8 - MASALA DO WHILE

// let a = 2;
// let b = 5;
// let product = 1;
// let i = a;

// do {
//   product *= i;
//   i++;
// } while (i <= b);

// console.log("Ko‘paytma:", product);

//================================================================================== 9 -MASALA

// let a = 1;
// let b = 4;
// let sum = 0;

// for (let i = a; i <= b; i++) {
//   sum += i * i;
// }

// console.log("Kvadratlar yig'indisi:", sum);

//================================================================================== 9 - MASALA  WHILE

// let a = 1;
// let b = 4;
// let sum = 0;
// let i = a;

// while (i <= b) {
//   sum += i * i;
//   i++;
// }

// console.log("Kvadratlar yig'indisi:", sum);

//================================================================================== 9 - MASALA DO WHILE

// let a = 1;
// let b = 4;
// let sum = 0;
// let i = a;

// do {
//   sum += i * i;
//   i++;
// } while (i <= b);

// console.log("Kvadratlar yig'indisi:", sum);

//================================================================================== 10 -MASALA

// let n = 5;
// let S = 0;

// for (let i = 1; i <= n; i++) {
//   S += 1 / i;
// }

// console.log("Yig'indi S =", S.toFixed(4));

//================================================================================== 10 -MASALA WHILE

// let n = 5;
// let S = 0;
// let i = 1;

// while (i <= n) {
//   S += 1 / i;
//   i++;
// }

// console.log("Yig'indi S =", S.toFixed(4));

//================================================================================== 10 -MASALA DO WHILE

// let n = 5;
// let S = 0;
// let i = 1;

// while (i <= n) {
//   S += 1 / i;
//   i++;
// }

// console.log("Yig'indi S =", S.toFixed(4));

// Orginali

// function Car(make, speed) {
//   this.make = make;
//   this.speed = speed;
//   this.accelerate = function () {
//     console.log(`${this.make} tezligi ${(this.speed += 10)}km/h ga oshdi.`);
//   };
//   this.brake = function () {
//     console.log(`${this.make} tezligi ${(this.speed -= 5)}km / h ga kamaydi.`);
//   };
// }

// let car1 = new Car("BMW", 120);
// let car2 = new Car("Mercedes", 95);

//Protatypega ugirilgani

// function Car(make, speed) {
//   this.make = make;
//   this.speed = speed;
// }

// Car.prototype.speed = function () {
//     console.log(`${this.make} tezligi ${(this.speed)} km/h oshdi`);

// };

// (function(){
//   console.log("Assalomu alaykum");

// })()

// (function () {
//   const massages = "salom dunyo";
//   console.log(massages);
// })();

// (
//   function(name){
//     const fullName=`salom mening ismim ${name}`
//     console.log(fullName);

//   }
// )("Asilbek")

// const a = 10;

// (function () {
//   const a = 20;
//   console.log(`ichki qiymat: ${a}`);
// })();

// console.log(`tashqi qiymat: ${a}`);

// console.log(Number("123"));
// console.log(Number("123.07"));
// console.log(Number("abc"));

// console.log(Number("123px"));
// console.log(Number("123.45px"));
// console.log(Number("abc123"));

// console.log(isNaN(NaN));
// console.log(isNaN("abc"));
// console.log(isNaN(123 ));

// console.log(isFinite(123));
// console.log(isFinite(Infinity));
// console.log(isFinite("123"));
// console.log(isFinite("abc"));

// console.log(Number.isInteger(123));
// console.log(Number.isInteger(123.45));
// console.log(Number.isInteger("123"));

// const userInput = prompt("iltimos biror raqam kiriting ");
// if (!isNaN(userInput)) {
//   console.log(`siz kiritgan son: ${Number(userInput)}`);
// } else {
//   console.log("Bu son emas!");
// }

// const userInp = prompt("Iltimos, butun son kiriting: ");
// if (Number.isInteger(Number(userInp))) {
//   console.log("Bu butun son!");
// } else {
//   console.log("Bu butun son emas!");
// }

// console.log(Math.floor(5.7));
// console.log(Math.floor(-5.7));

// console.log(Math.ceil(5.7));
// console.log(Math.ceil(-5.7));

// console.log(Math.round(4.5));
// console.log(Math.round(4.4));

// console.log(Math.trunc(4.9));
// console.log(Math.trunc(-4.9));

// let minSon = Math.min(5, 10, 2, 8);
// console.log(minSon);  // Natija: 2

// let maxSon = Math.max(5, 10, 2, 8);
// console.log(maxSon);  // Natija: 10

// let natija = Math.pow(2, 3);
// console.log(natija);  // Natija: 8

// let ildiz = Math.sqrt(16);
// console.log(ildiz);  // Natija: 4

// let kubIldiz = Math.cbrt(27);
// console.log(kubIldiz);  // Natija: 3

// let number = 1000000000
// let price = 25000.00
// let byte = 234567869708
// // Bu holatda raqamlarni o'qish siz uchun qiyinlik qilishi aniq shuning uchun
// let numbers = 1_000_000_000
// let prices = 25_000.00
// let bytes = 2_345_678_697.08

// let largeNumber = 1_000_000_000_000_000
// console.log(largeNumber);

// let currentDate = new Date();
// console.log(currentDate);

// let specificDate = new Date('2025-02-06T16:00:00');
// console.log(specificDate);

// let customDate = new Date(2020, 1, 7);
// console.log(customDate);

// let date = new Date().toLocaleTimeString()
// console.log(date);

// let date = new Date()
// console.log(date);

// let rusDate = new Intl.DateTimeFormat("ru-RU").format(date);
// console.log(rusDate);

// let date = new Date();

// let shortDate = new Intl.DateTimeFormat(
//   "en-US",
//   {
//     year: "numeric",
//     month: "long",
//     day: "numeric",
//   }
// ).format(date);

// console.log(shortDate);

// let date =new Date()

// let  timeFormat = new Intl.DateTimeFormat('en-US',{
//     hour:"2-digit",
//     minute:"2-digit",
//     second: "2-digit",
//     hour12: false
// }).format(date)
// let  timeFormat24 = new Intl.DateTimeFormat('en-US',{
//     hour:"2-digit",
//     minute:"2-digit",
//     second: "2-digit",
//     hour12: true
// }).format(date)

// console.log(timeFormat, timeFormat24);

// const user = {
//   names: "Asilbek",
//   firstName: "Suvonov",
//   age: 17,
//   greet: function () {
//     console.log(
//       `salom mening ismim ${this.names}, familiyam esa ${this.firstName} va yoshim ${this.age}`
//     );
//   },
// };
// user.greet();7

// function Car(model, year, color) {
//   this.model = model;
//   this.year = year;
//   this.color = color;

//   this.start = function () {
//     console.log(`${this.model} ishlayabdi...`);
//   };
// }

// //obyektlarni yaratish
// let car1 = new Car("Malibu", 2020, "oq");
// let car2 = new Car("Nexia", 2026, "oq");

// car1.start();
// car2.start();

// console.log(car1.color);
// console.log(car2.year);

// function Person(name, age) {
//   this.name = name;
//   this.age = age;
// }

// const person1 = new Person("Asilbek", 17);
// console.log(person1);

// console.log(Person.prototype);

// function Person(name, age) {
//   this.name = name;
//   this.age = age;
// }

// Person.prototype.greet = function () {
//   console.log(`Salom mening ismim ${this.name}, yoshim ${this.age}`);
// };
// const person1 = new Person("Asilbek", 17);
// const person2 = new Person("Azizbek", 15);

// person1.greet();
// person2.greet();






// function Car(make, speed) {
//   this.make = make;
//   this.speed = speed;
//   this.accelerate = function () {
//     console.log(`${this.make} tezligi ${this.speed += 10} km / h ga oshdi `);
//   };
//   this.break = function () {
//     console.log(`${this.make} tezligi ${this.speed -= 5} km / h ga kamaydi`);
//   };
// }

// let car1 = new Car(`BMW`, 120);
// let car2 = new Car(`Mercedes`, 200);

// car1.accelerate()
// car2.accelerate()

// car1.break()
// car2.break()



function Car(make, speed) {
  this.make = make;
  this.speed = speed;
}

// Prototypelarga metodlarni qo‘shamiz
Car.prototype.accelerate = function () {
  this.speed += 10;
  console.log(`${this.make} tezligi ${this.speed} km/h ga oshdi`);
};

Car.prototype.brake = function () {
  this.speed -= 5;
  console.log(`${this.make} tezligi ${this.speed} km/h ga kamaydi`);
};

// Ob'ektlar yaratamiz
let car1 = new Car("BMW", 120);
let car2 = new Car("Mercedes", 200);

// Metodlarni chaqiramiz
car1.accelerate(); // BMW tezligi 130 km/h ga oshdi
car2.accelerate(); // Mercedes tezligi 210 km/h ga oshdi
car1.brake();      // BMW tezligi 125 km/h ga kamaydi
car2.brake();      // Mercedes tezligi 205 km/h ga kamaydi
